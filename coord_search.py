...
with open('pickle/data_final.pickle', 'rb') as f:
   data_new = pickle.load(f)

data_new=pd.DataFrame(data_new,columns=['Вуз','Адрес','e-mail','Телефон','Сайт','Категории'])
data_new['Адрес']=data_new['Адрес'].apply(lambda x: x.replace('/',' '))
...
with open('pickle/data_final_2.pickle', 'wb') as f:
...
data_new.to_csv("csv_files/output_final.csv",
          sep = ";")